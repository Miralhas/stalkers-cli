...
def check_novel_count(novel: Path, url: str, index: int, total: int):
    try:
        ...
        if (downloaded_count != source_chapters):
            print(f"[bright_white][{index+1}/{total}][/bright_white][red]Chapters mismatch: {novel.name}")
            return True
        else:
            print(f"[bright_white][{index+1}/{total}][/bright_white][green]Novel {novel.name} downloaded successfully!")
            return False
    except:
        print(f"[bright_white][{index+1}/{total}][/bright_white][red]Couldn't extract chapters: {novel.name}")
        return True
...
def check_all_novels_count(root_path:Path):
    ...
    for index, novel in enumerate(novels):
        ...
        if url is not None and check_novel_count(novel, url, index, total):
